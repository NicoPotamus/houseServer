...
// Create account
export async function createAccount(pool, user) {
    const hash = await bcrypt.hash(user.password, 10);
    const result = await pool.query('INSERT INTO users (name, email, password, storage_quota, files) VALUES ($1, $2, $3, $4, $5) RETURNING *', [user.name || null, user.email, hash, user.storage_quota || 1073741824, user.files || null]);
...
export async function updateAccount(pool, id, updates) {
    const fields = [];
    const values = [];
    let idx = 1;
...
export async function upsertUserServer(pool, userId, deviceId, status = 'online') {
    await pool.query(`
...
export async function upsertUserServerByUsername(pool, username, deviceId) {
...
export async function verifyUserCredentials(pool, username, password) {
    const user = await getUserByUsername(pool, username);
    if (!user)
        return false;
    const valid = await bcrypt.compare(password, user.password);
