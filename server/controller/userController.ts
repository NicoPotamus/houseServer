...
import { User, LoginRequestBody } from '../model/types.js';
import fs from 'fs';
...
export const createUser: RequestHandler = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'INSERT INTO users (name, email, storage_quota) VALUES ($1, $2, $3) RETURNING *',
            [name, email, storage_quota]
        );
...
export const updateUser: RequestHandler = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'UPDATE users SET name = $1, email = $2, storage_quota = $3 WHERE id = $4 RETURNING *',
            [name, email, storage_quota, req.params.id]
        );
        if (result.rows.length === 0) {
            res.status(404).json({ message: 'User not found' });
            return;
...
// Login function
export const loginUser = async (req: Request<{}, {}, LoginRequestBody>, res: Response): Promise<void> => {
...
export const ensureUserPath = async (username: string): Promise<void> => {
