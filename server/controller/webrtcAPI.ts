...
import * as diskController from './diskController.js';
...
// Define types for WebRTC API handling
...
export async function handleDataChannelMessage(message: any, dataChannel: any) {
  ...
  const { id, endpoint, method, params } = message as WebRTCRequest;
...
function sendSuccessResponse(dataChannel: any, id: number, result: any) {
...
function sendErrorResponse(dataChannel: any, id: number, message: string, code = 500) {
  const response: WebRTCResponse = {
    type: 'response',
    id,
    error: { message, code }
...
async function handleFileEndpoint(endpoint: string, method: string, params: any, id: number, dataChannel: any) {
  try {
...
async function handleDeviceInfoEndpoint(id: number, dataChannel: any) {
  try {
    const os = require('os');
 ...
 */
async function listFilesForWebRTC(folderPath: string) {
