...
export const createUserVolume = (userId: string): void => {
...
export const getUserStorageUsage = (userId: string): number => {
    const volumeName = `user_${userId}`;
    try {
...
export const enforceUserQuota = (userId: string, fileSize: number, userQuota: number): boolean => {
